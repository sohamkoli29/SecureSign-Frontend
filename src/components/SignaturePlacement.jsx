...
const SignaturePlacement = ({ document, onComplete }) => {
  ...
  const [finalizedUrl, setFinalizedUrl] = useState(
    document.signed_file_url || null,
  );
  ...
  const fetchSignatures = async () => {
    try {
      const { data } = await axios.get(
        `/api/signatures/document/${document.id}`,
        auth(),
      );
      setSignatures(data.data || []);
    } catch (e) {
      console.error("fetchSignatures:", e);
    }
  };
  ...
  const handleFinalize = async () => {
    ...
    if (n === 0) {
      alert("Sign at least one signature before finalizing.");
      return;
    }
  ...
  const openSendModal = (sig) => {
    ...
    setSendName(
      target && target.signer_name !== "Signer" ? target.signer_name : "",
    );
  ...
  const handleSendLink = async () => {
    ...
    if (!target?.id) {
      alert(
        "No pending signature placeholder found. Add a signature placeholder first.",
      );
      return;
    }
      ...
      <aside className="w-72 shrink-0 bg-[#16213e] border-r border-white/10 flex flex-col">
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/10">
          <div>
            ...
            <p className="text-xs text-white/40 mt-0.5 truncate max-w-[160px]">
              {document.title}
            </p>
          </div>
          <button
              ...
              {!sentLink ? (
                <>
                  ...
                  {signatures.filter((s) => s.status !== "signed").length >
                    ...
                    <div>
                      ...
                      >
                        <option value="">
                          Select a signature placeholder…
                        </option>
                        {signatures
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">
                      ...
                      <span className="text-gray-400 font-normal">
                        (optional — sends email if provided)
                      </span>
                    </label>
                    <input
                      type="email"
                  ...
                  <div className="flex gap-2.5 p-3 bg-blue-50 rounded-lg border border-blue-100">
                    ...
                    <p className="text-xs text-blue-700">
                      The link lets anyone with it sign this document. It
                      expires in 7 days. Share only with the intended signer.
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-gray-400">
                    ...
                    <strong className="text-gray-600">{sendName}</strong>. They
                    can sign the document without creating an account. Link
                    expires in 7 days.
                  </p>
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-3 hover:border-white/20 transition">
      <div className="flex items-center justify-between mb-2">
        ...
        <span
          className={`text-xs px-2 py-0.5 rounded-full border font-medium ${statusStyle}`}
        >
          {sig.status}
        </span>
      </div>
      ...
      {sig.signature_data && (
        <div
          className="mb-2 rounded-lg bg-white/10 flex items-center justify-center"
          style={{ height: 44 }}
        >
          <img
      ...
      <div className="flex items-center justify-between mb-2.5">
        ...
        <span className="text-xs text-white/25 font-mono">
          {Math.round(sig.coordinates?.x ?? 0)},{" "}
          {Math.round(sig.coordinates?.y ?? 0)}
        </span>
      </div>
